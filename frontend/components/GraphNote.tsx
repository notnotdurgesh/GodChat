...
export const GraphNoteComponent: React.FC<GraphNoteProps> = ({
    ...
    const [prevNoteLayout, setPrevNoteLayout] = useState({
        x: note.x,
        y: note.y,
        width: note.width,
        height: note.height
    ...
    if (!isInteracting && (
        note.x !== prevNoteLayout.x || 
        note.y !== prevNoteLayout.y || 
        note.width !== prevNoteLayout.width || 
        note.height !== prevNoteLayout.height
    )) {
        setPrevNoteLayout({ x: note.x, y: note.y, width: note.width, height: note.height });
    ...
    const handleResizeStart = (e: React.MouseEvent, handle: string) => {
        ...
        const handleMouseUp = (mv: MouseEvent) => {
            ...
            const finalUpdate: Partial<GraphNote> = { resizeMode: 'FIXED' };
