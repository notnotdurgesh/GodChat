...
import { AuthenticatedUser } from '../services/backendService';
...
const areSidebarPropsEqual = (prev: SidebarProps, next: SidebarProps) => {
  ...
  if (prev.currentUser.lastSignedInAt !== next.currentUser.lastSignedInAt) return false;
  if (prev.currentUser.currentSessionLastUsedAt !== next.currentUser.currentSessionLastUsedAt) return false;
    ...
    <div className="w-full h-full bg-surface border-r border-border flex flex-col shrink-0 relative transition-colors duration-300 select-none">
      ...
      <div className="p-4 z-10 flex flex-col gap-3 border-b border-border/40 shrink-0">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="p-3 rounded-xl border border-border bg-surface hover:bg-black/5 dark:hover:bg-white/5 text-text-secondary hover:text-text-primary transition-all active:scale-95 hover:border-text-secondary/30 flex items-center justify-center shrink-0"
            title="Create Folder"
          >
            <FolderPlus size={18} />
          </button>
      ...
      <div className="p-3 border-t border-border mt-auto z-10">
        <button
          ...
          className="w-full rounded-2xl border border-border bg-surface shadow-sm overflow-hidden flex items-center justify-between p-2.5 transition-all hover:bg-black/5 dark:hover:bg-white/5"
          title="Profile & Settings"
        >
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <div className="relative w-11 h-11 rounded-xl bg-accent-primary text-white flex items-center justify-center text-sm font-bold shadow-sm shrink-0">
              {currentUser.username.slice(0, 2).toUpperCase()}
              <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-500 border-2 border-surface" />
            </div>
            <div className="min-w-0 flex-1 text-left">
              <div className="text-sm font-bold text-text-primary truncate">{currentUser.username}</div>
              <div className="text-[11px] font-medium text-text-secondary truncate mt-0.5">Settings & Profile</div>
            </div>
          </div>
          <div className="flex items-center justify-center w-8 h-8 rounded-lg text-text-secondary/60 shrink-0">
            <Settings size={18} />
          </div>
        </button>
      </div>
